...
```{r}
siteSf = st_as_sf(siteData, coords=c('easting', 'northing'), crs='+proj=utm +zone=10 +ellps=GRS80 +datum=NAD83 +units=m +no_defs')
...
Note I am creating a temporary file called tmp in which I have projected the Oregon state boundary into the same CRS as the elevation data. In general, it is much faster to reproject shapefiles than raster files, especially large raster files.
```{r}
